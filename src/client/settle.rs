...
impl ArkClient {
        ...
        R: Rng + CryptoRng,
    {
        if onchain_inputs.is_empty() && vtxo_inputs.is_empty() {
            bail!("cannot join batch without inputs");
        }
        ...
        //     self.inner.timeout,
        //     self.grpc_client.register_intent(intent),
        // )
        //     .await
        //     .context("failed to register intent")??;
        let intent_id = self.grpc_client.register_intent(intent).await?;
        ...
        let network_client = &self.grpc_client;
            ...
            match stream.next().await {
                Some(Ok(event)) => match event {
                        ...
                        match e.batch_tree_event_type {
                            BatchTreeEventType::Vtxo => {
                                match &mut vtxo_graph_chunks {
                                    ...
                                    None => {
                                        bail!("received unexpected VTXO graph chunk",)
                                    }
                                };
                            }
                            BatchTreeEventType::Connector => {
                                match connectors_graph_chunks {
                                    ...
                                    None => {
                                        bail!("received unexpected connectors graph chunk",)
                                    }
                                };
                            }
                        }
                        ...
                        match e.batch_tree_event_type {
                            BatchTreeEventType::Vtxo => {
                                match vtxo_graph {
                                    ...
                                    None => {
                                        bail!("received batch tree signature without TX graph",);
                                    }
                                };
                            }
                            BatchTreeEventType::Connector => {
                    ...
                    StreamEvent::TreeSigningStarted(e) => {
                        ...
                        let chunks = vtxo_graph_chunks.take().ok_or(anyhow!(
                    ...
                    StreamEvent::TreeNonces(e) => {
                        ...
                        if agg_nonce_pks.len() == vtxo_graph.nb_of_nodes() {
                            let cosigner_kp = own_cosigner_kps
                                ...
                                .ok_or_else(|| anyhow!("no cosigner keypair to sign for own PK"))?;

                            let our_nonce_trees = our_nonce_trees
                                .as_mut()
                                .ok_or(anyhow!("missing nonce trees during batch protocol"))?;

                            let our_nonce_tree = our_nonce_trees
                                .get_mut(cosigner_kp)
                                .ok_or(anyhow!("missing nonce tree during batch protocol",))?;

                            let unsigned_commitment_tx = unsigned_commitment_tx
                                .as_ref()
                                .ok_or_else(|| anyhow!("missing commitment TX"))?;

                            let batch_expiry =
                                batch_expiry.ok_or_else(|| anyhow!("missing batch expiry"))?;
                            ...
                            for (txid, _) in vtxo_graph.as_map() {
                                let agg_nonce_pk = agg_nonce_pks.get(&txid).ok_or_else(|| {
                                    anyhow!(format!("missing aggregated nonce PK for TX {txid}"))
                                })?;
                    ...
                    StreamEvent::BatchFinalization(e) => {
                        ...
                        let signed_forfeit_psbts = if !vtxo_inputs.is_empty() {
                            let chunks = connectors_graph_chunks.take().ok_or(anyhow!(
                            ...
                            create_and_sign_forfeit_txs(
                                ...
                                |msg, vtxo| {
                                    ...
                                    let kp = if ark_address
                                        == main_address.to_ark_address().encode()
                                    {
                                        main_signing_kp
                                    } else {
                                        let maybe_kp =
                                            self.game_addresses.iter().find_map(|game_address| {
                                                if game_address.vtxo.to_ark_address().encode()
                                                    == ark_address
                                                {
                                                    Some(
                                                        game_address.secret_key.keypair(&self.secp),
                                                    )
                                                } else {
                                                    None
                                                }
                    ...
                    StreamEvent::BatchFailed(ref e) => {
                        if Some(&e.id) == batch_id.as_ref() {
                            return Err(anyhow!(format!("batch failed {}: {}", e.id, e.reason)));
                        }
                ...
                None => {
                    bail!("dropped batch event stream");
                }
            }
        }
    ...
    async fn fetch_commitment_transaction_inputs(
        ...
        for (_vtxo, virtual_tx_outpoints) in spendable_vtxos.iter() {
