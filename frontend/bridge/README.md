# @bonomat/satsday-wallet-bridge

Type-safe communication bridge between Satsday iframe and parent wallet applications.

## Installation

```bash
npm install @bonomat/satsday-wallet-bridge
# or
pnpm add @bonomat/satsday-wallet-bridge
# or
yarn add @bonomat/satsday-wallet-bridge
```

## Overview

This library provides a secure, type-safe communication layer between the Satsday application (embedded in an iframe)
and parent wallet applications. It handles message passing, request/response cycles, and error handling for wallet
operations.

## Usage

### For Iframe (SatsdayApp)

Use `SatsdayClient` to request wallet operations from the parent window:

```typescript
import {SatsdayClient} from "@bonomat/satsday-wallet-bridge";

// Create a client instance
const client = new SatsdayClient();

```

### For Parent Wallet

Use `WalletProvider` to handle requests from the Satsday iframe:

```typescript
import {AddressType, WalletProvider} from "@bonomat/satsday-wallet-bridge";
...
The bridge includes a capabilities discovery system that allows the iframe to query what features the wallet supports.
This enables better UX by hiding/disabling unavailable features.
...
Client for the Satsday iframe to communicate with parent wallet.
...
- `getApiKey(): Promise<string>` - Get the Satsday API key
...
Provider for parent wallets to handle requests from Satsday iframe.

#### Constructor

```typescript
new WalletProvider(handlers
:
WalletHandlers, allowedOrigins ? : string[]
)
```
...
This package is published to npm as `@bonomat/satsday-wallet-bridge`.
...
```bash
# Build the package
pnpm --filter @bonomat/satsday-wallet-bridge build

# Publish the package
pnpm --filter @bonomat/satsday-wallet-bridge publish --access public
```
