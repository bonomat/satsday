...
export default function HowItWorksContent() {
  const steps = [
    ...
    },
    {
      ...
      detail:
        "Your transaction hash + our server seed = provably fair randomness.",
    },
    {
      icon: Dice6,
      title: "Result Generation",
      description:
        "The combined hash generates a number from 1-65535 for the dice result.",
      detail:
        "Mathematical algorithms convert the hash into a fair dice roll you can verify.",
    },
    {
      icon: CheckCircle,
      title: "Verification",
      description:
        "You can verify the fairness of every single roll independently.",
      detail:
        "All data is provided so you can recreate and verify the result yourself.",
    },
  ];
            ...
            >
              <CardHeader className="text-center">
                <div className="w-16 h-16 bg-primary/10 rounded-full flex items-center justify-center mx-auto mb-4 relative">
                  ...
                  <Badge className="absolute -top-2 -right-2 w-6 h-6 rounded-full p-0 flex items-center justify-center bg-primary text-primary-foreground">
                    {index + 1}
                  </Badge>
                </div>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="bg-muted/30 p-4 rounded-lg">
            <p className="text-sm text-muted-foreground text-center">
              <strong>Formula:</strong> HMAC-SHA256(client_tx, server_seed) →
              Dice Result (1-65535)
            </p>
          </div>
        </CardContent>
